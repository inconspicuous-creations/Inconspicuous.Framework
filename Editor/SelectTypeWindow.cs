...
namespace Inconspicuous.Framework {
	public class SelectTypeWindow : EditorWindow {
		...
		public static void Open(Type searchType, Action<Type> callback) {
		...
		public static event Action<Type> onSelected;
			...
			switch(Event.current.type) {
				case EventType.used:
					if(Event.current.clickCount == 2) {
						onSelected(ICEditorUtility.GetType(results[selected]));
						onSelected = null;
						Close();
					}
					break;
